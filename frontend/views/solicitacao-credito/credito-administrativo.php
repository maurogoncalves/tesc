    ...
    <div class="col-md-12">
        <div class="box box-solid">
            <div class="box-header with-border">
                ...
                <h4>Número do Cartão Administrativo: <?= $model->numeroCartaoAdministrativo ?></h4>
                <h4>Valor Solicitado: <?= \Yii::$app->formatter::DoubletoReal($model->creditoAdministrativo) ?></h4>
                ...
                <?php if($model->saldoRemanescente): ?>
                    <h4>Saldo Remanescente: <?= \Yii::$app->formatter::DoubletoReal($model->saldoRemanescente) ?></h4>
                <?php endif; ?>

                <?php if($model->valorTransferido): ?>
                    <h4>Valor a ser transferido: <?= \Yii::$app->formatter::DoubletoReal($model->valorTransferido) ?></h4>
                <?php endif; ?>
                            ...
                            case SolicitacaoCredito::STATUS_DEFERIDO: 
                                if (Usuario::permissoes([Usuario::PERFIL_SUPER_ADMIN, Usuario::PERFIL_TESC_PASSE_ESCOLAR])) { ?>
                                  <?php $form = ActiveForm::begin([
                                   ...
                                   <div class="row">
                                        <?php if(!$model->saldoRemanescente && $model->tipoSolicitacao == SolicitacaoCredito::TIPO_CREDITO_ADMINISTRATIVO): ?>
                                        <div class="col-md-2">
                                            <?= $form->field($model, 'saldoRemanescente')->textInput(['id' => 'saldoRemanescente','maxlength' => true, 'class' => 'form-control money']); ?>
                                        </div>  
                                        <?php  endif; ?>
                                        <?php if(!$model->valorTransferido): ?>
                                        <div class="col-md-2">
                                            <?= $form->field($model, 'valorTransferido')->textInput(['id' => 'valorTransferido','readonly' => true,'maxlength' => true, 'class' => 'form-control money']); ?>
                                        </div>
                                        <?php  endif; ?>
                                        <?php if((!$model->valorTransferido || !$model->dataTransferencia) || (!$model->saldoRemanescente && $model->tipoSolicitacao == SolicitacaoCredito::TIPO_CREDITO_ADMINISTRATIVO)): ?>
                                            <?php if(!$model->dataTransferencia): ?>
                                       <div class="col-md-2">
                                            <?php 
...
<script>
let solicitado = 0;
solicitado = <?= $model->creditoAdministrativo ?>;
$("#saldoRemanescente").keyup(function(){
   // setTimeout(() => habilitarTransferencia(), 500)
   habilitarTransferencia()
   calcularTotal()
})
$("#saldoRemanescente").change(function(){
    habilitarTransferencia()
       // setTimeout(() => habilitarTransferencia(), 500)
       let saldoRemanescente = getSaldoRemanescente();
    let valorASerTransferido = getValorSerTransferido()
    if(valorASerTransferido < 0){
        $("#saldoRemanescente").val("")
        $("#saldoRemanescente").focus()
        $("#valorTransferido").val("")
      
        return Swal.fire(
                '',
                'O saldo remanescente não deve ser maior que o valor solicitado pela unidade escolar',
                'error'
            )
    }
    ...
    return Swal.fire(
                '',
                'O valor do saldo remanescente deve ser positivo',
                'error'
            )
...
$("#valorTransferido").change(function(){
   // setTimeout(() => habilitarTransferencia(), 500)
    let saldoRemanescente = getSaldoRemanescente();
    let valorASerTransferido = getValorSerTransferido()
    if(valorASerTransferido < 0){
...
function habilitarTransferencia(){
    ...
    if(valorTransferido.value && dataTransferencia.value && saldoRemanescente.value){
        $("#salvarTransferencia").prop('disabled', false);
    }
}
function getSaldoRemanescente(){
    return BRLtoReal($("#saldoRemanescente").val());;
}
function getValorSerTransferido(){
    return BRLtoReal($("#valorTransferido").val());;
}

function  calcularTotal(){
    let total = 0;
    let saldoRemanescente = BRLtoReal($("#saldoRemanescente").val());
    total = saldoRemanescente - solicitado;
    $("#valorTransferido").val(realToBRL(total *-1))
