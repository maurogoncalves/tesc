...
<div class="box-body">
    ...
    <?= $form->field($model, 'idSolicitacaoTransporte')->hiddenInput(['maxlength' => true])->label(false); ?> 

    
    <div class="row" id="justificativa">
         <div class="col-md-12">
         <?php 
            if(Usuario::permissoes([Usuario::PERFIL_SUPER_ADMIN, Usuario::PERFIL_TESC_DISTRIBUICAO, Usuario::PERFIL_TESC_PASSE_ESCOLAR, Usuario::PERFIL_DIRETOR, Usuario::PERFIL_DRE]) && 
             (SolicitacaoTransporte::STATUS_INDEFERIDO == $status) && !isset($alterarDadosProtegidos)  
              ){
                 echo $form->field($model, 'justificativa')
                    ->dropDownList(
                        [
                            'Distância' => 'Distância',
                            'Dados Divergentes SED' => 'Dados Divergentes SED',
                            'Preenchimento Incompleto' => 'Preenchimento Incompleto',
                            'Inscrição/Declaração' => 'Inscrição/Declaração', 
                            'Outros' => 'Outros',
                            // 'Transferência' => 'Transferência',
                            // 'Desistência' => 'Desistência',
                            // 'Mudança de Endereço' => 'Mudança de Endereço',
                            // 'Mudança de Período' => 'Mudança de Período',
                            // 'Documentação Incompleta' => 'Documentação Incompleta',
                            // 'Vaga em Escola mais Próxima' => 'Vaga em Escola mais Próxima',
                            // 'Matrícula Irregular' => 'Matrícula Irregular',
                            // 'Falecimento' => 'Falecimento',
                            // 'Distância Inferior 2 Km' => 'Distância Inferior 2 Km',
                        ],          
                        ['prompt'=>'Selecione']    
                    );
                    // if(Usuario::permissoes([Usuario::PERFIL_SUPER_ADMIN, Usuario::PERFIL_TESC_DISTRIBUICAO, Usuario::PERFIL_TESC_PASSE_ESCOLAR]))
                        echo $form->field($model, 'justificativaSetor')->textarea(['rows' => '6','placeholder' => '']);
